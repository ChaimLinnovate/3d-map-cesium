...
export const SensorFrustum = ({
  ...
  const outlinePositions = useMemo(() => {
    ...
    const halfHFOV = hfovRad / 3.3;
    const halfVFOV = vfovRad / 3.4;
  ...
  return (
    <Primitive
      key={`${point.lat}-${point.lon}-${point.hae ?? 0}-${azimuth}-${elevation}-${hfov}-${vfov}-${range}`}
